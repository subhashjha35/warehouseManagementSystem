...
<?php
    ...
    if(isset($_POST['x1'])):
        ...
        if($source_img!=FALSE){
            ...
            $rowAffected=$whm->updateImage($curr_wm,$cropped_image_dest);
            if(!$rowAffected){
    ...
    if(isset($_FILES['user_image'])):
        ...
        if($size>40960):
            ...
            if(move_uploaded_file($tmp,$dest)):
                ...
                if($res=$whm->updateImage($curr_wm,$dest)):
                    ?>
                    <script type="text/javascript">
                        $(document).ready(function(){
                            $('#my_modal').modal('show');
    ...
    if(isset($_POST['profile_data'])):
        ...
        $wm_pass=$_POST['wm_pass'];
        $wm_id=$_POST['wm_id'];
        $wm_designation=$_POST['wm_designation'];
        $email=$_POST['email'];
        $contact_no=$_POST['contact_no'];
        $warehouse_name=$_POST['warehouse_name'];
        $warehouse_location=$_POST['warehouse_location'];
        $data=array('wm_name'=>$wm_name,
            'wm_pass'=>$wm_pass,
            'wm_designation'=>$wm_designation,
            'email'=>$email,
            'contact_no'=>$contact_no,
            'warehouse_name'=>$warehouse_name,
            'warehouse_location'=>$warehouse_location);
        /*print_r($data);*/
        $whm->updateDetails($curr_wm,$data);
    endif;
?>
                ...
                <div class="main_container">
                    <div class="row">
                        ...
                        <div class="col-lg-6 text-lg-right">
                            <button class="btn btn-secondary" onclick="enable_edit_profile()"><span class="fa fa-pencil"></span>Edit Your Profile</button>
                        </div>
                    </div>
                    <hr>
                    <div class="row">
                            ...
                            <fieldset id="profile_form">
                                <form method="post" action="<?=$_SERVER['PHP_SELF'];?>" class="" >
                                    <div class="form-group">
                                        <label for="wm_id">Warehouse Manager ID</label>
                                        <input type="text" name="wm_id" class="form-control" value="<?=$sessionWarehouseUser['wm_id'];?>">
                                    </div>
                                    <div class="form-group">
                                        <label for="wm_pass">Warehouse Manager Password</label>
                                        <input type="text" name="wm_pass" class="form-control" value="">
                                    </div>
                                    <div class="form-group">
                                        <label for="wm_name">Your Name</label>
                                        <input type="text" name="wm_name" class="form-control" value="<?=$sessionWarehouseUser['wm_name'];?>">
                                    </div>
                                    <div class="form-group">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="wm_designation" class="form-control" value="<?=$sessionWarehouseUser['wm_designation'];?>">
                                    </div>
                                    <div class="form-group">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="warehouse_name" class="form-control" value="<?=$sessionWarehouseUser['warehouse_name'];?>">
                                    </div>
                                    <div class="form-group">
                                        <label for="name">Address</label>
                                        <textarea class="form-control" name="warehouse_location"><?=$sessionWarehouseUser['warehouse_location'];?></textarea>
                                    </div>
